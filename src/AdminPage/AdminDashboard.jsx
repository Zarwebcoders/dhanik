...
const AdminDashboard = () => {
    ...
    const fetchAdminData = async () => {
        try {
            ...
            const [statsRes, usersRes, txRes, setRes] = await Promise.all([
                ...
                fetch(`${BASE_URL}/api/admin/settings`, { headers })
            ]);
    ...
    const handleSaveUserEdit = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${BASE_URL}/api/admin/users/${user._id}/edit`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                                                ...
                                                <tr key={tx._id || i}>
                                                    ...
                                                    <td>
                                                        {tx.paymentScreenshot ? (
                                                            <button className="btn-outline-small" onClick={() => setProofImage(`${BASE_URL}/uploads/` + tx.paymentScreenshot)}>View Proof</button>
                                                        ) : '—'}
                                                    </td>
                                                    <td>
                                                        <div style={{ display: 'flex', gap: '8px' }}>
